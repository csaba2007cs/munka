...
function load_state(string $path): array
{
    ...
    $state = is_array($decoded) ? $decoded : default_state();

    return ensure_mobilmozi_defaults($state);
}
...
if ($method === 'POST') {
    ...
    $current = load_state($stateFile);
    $merged = merge_state($current, $patch);
    $merged = apply_quiz_answer_lock($current, $patch, $merged);
    $merged = apply_hardware_event_log($merged, $patch);
    $merged = ensure_mobilmozi_defaults($merged);

    if (!save_state($stateFile, $merged)) {
        http_response_code(500);
        echo json_encode(['error' => 'Failed to persist state'], JSON_UNESCAPED_UNICODE);
        exit;
    }
